...
public class Plotter2410 : MonoBehaviour {
 ...
 public GameObject CanvasError2;
...
// Assign column name from columnList to Name variables
 ...
 for (var i = 0; i < pointList.Count; i++)
	...
	scaleXZ = System.Convert.ToSingle((plotScale/2)/columnList.Count); //any value possible to set boldness of each bar
...
// Assign column name from columnList to Name variables
 ...
 for (var i = 0; i < pointList.Count; i++)
	...
	GameObject dataBar = Instantiate(BoxPrefab, new Vector3(height/2,position, 0), Quaternion.identity);

	Debug.Log(i+" : "+System.Convert.ToSingle(pointList[i][xName]));
	dataBar.transform.localScale = new Vector3 (height/2,scaleXZ,scaleXZ);
